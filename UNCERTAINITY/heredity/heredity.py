...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    prob=1
    for person in people:
        genes_person=get_no_of_genes(person)
        person_has_trait=person in have_trait
        if not people[person]["father"]:
            prob*=PROBS["gene"][genes_person]

        else:
            father_genes=get_no_of_genes(people[person]["father"])
            mother_genes=get_no_of_genes(people[person]["mother"])
            father_prob=(father_genes*0.5)*(1-PROBS['mutation'])+(1-father_genes*0.5)*(PROBS['mutation'])
            mother_prob=(mother_genes*0.5)*(1-PROBS['mutation'])+(1-mother_genes*0.5)*(PROBS['mutation'])
            if genes_person==0:
                prob*=(1-father_prob)
                prob*=(1-mother_prob)
            elif genes_person==2:
        ...
        # Accounting traits
        prob*=PROBS['trait'][genes_person][person_has_trait]  
    return prob
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for person in probabilities:
        no_of_genes_person=get_no_of_genes(person)
        person_has_trait=person in have_trait
        probabilities[person]["gene"][no_of_genes_person]+=p
